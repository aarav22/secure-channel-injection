class Sha256:
    ...
    def update(self, m):
        ...
        self.mlen += len(m)
        print(f'len(m) = {len(m)}, self.mlen = {self.mlen}, self.buf = {self.buf}')
        m = self.buf + m

        for i in range(0, len(m) // 64):
            print("Inside length: ", len(m))
...
def my_test():
    import secrets, hashlib, random
    strings = [""] * 6
    # 13 bytes string: SQN + HDR
    strings[0] = secrets.token_bytes(13)
    # 51 bytes string: M_p1
    strings[1] =  secrets.token_bytes(51)
    # 13 bytes string: M_p2
    strings[2] = secrets.token_bytes(29)
    # 32 bytes string: M_*
    strings[3] =  secrets.token_bytes(32)
    # 9 bytes string: M_s1
    strings[4] = secrets.token_bytes(3)
    # 7 bytes string: M_s2
    strings[5] =  secrets.token_bytes(13)

    a, b = hashlib.sha256(), Sha256()
    a.update(strings[0] + strings[1])
    b.update(strings[0] + strings[1])
    a.update(strings[2] + strings[3] + strings[4])
    b.update(strings[2] + strings[3] + strings[4])
    a.update(strings[5])
    b.update(strings[5])
    a, b = a.hexdigest(), b.hexdigest()
    print(a, b)
